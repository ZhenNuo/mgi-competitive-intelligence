#!/bin/bash
# MGI 竞争情报三日报生成脚本 v3.0
# 用法：./run_tri_daily_report.sh [报告日期]

set -e

REPORT_DATE=${1:-$(date +%Y-%m-%d)}
WORKSPACE=~/Desktop/MGI-Competitive-Intelligence
TEMPLATE=$WORKSPACE/templates/三日报模板_v3.0.md
OUTPUT_DIR=$WORKSPACE/daily-reports
OUTPUT_MD=$OUTPUT_DIR/三日报_${REPORT_DATE}.md
OUTPUT_HTML=$OUTPUT_DIR/三日报_${REPORT_DATE}.html
...
START_DATE=$(date -v-3d -j -f "%Y-%m-%d" "$END_DATE" +%Y-%m-%d 2>/dev/null || date -d "3 days ago" +%Y-%m-%d)
...
if command -v pandoc &> /dev/null; then
    echo "📄 生成 HTML 版本..."
    pandoc "$OUTPUT_MD" -o "$OUTPUT_HTML" --metadata title="MGI 竞争情报三日报 $REPORT_DATE"
...
git commit -m "feat: 生成三日报 $REPORT_DATE" || true
